    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (response.code == HttpURLConnection.HTTP_UNAUTHORIZED && token != null) {
            ...
            if (refreshed) {
                ...
                // ВАЖНО: не чистим токен на любые ошибки (например, временная сеть).
                // Очистка происходит только если refresh явно невалиден (см. refreshToken()).
            }
        }
    ...
    private suspend fun refreshToken(): Boolean {
        return try {
            ...
            if (response.isSuccessful) {
                ...
                // Сервер вернул ошибку (например, 401 - невалидный refresh token)
                // Только в этом случае чистим локальную авторизацию.
